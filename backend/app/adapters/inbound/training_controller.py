    ...
    try:
        ...
        try:
            if not is_ai_generated:
                ...
                logger.info(f"🤖 AI_TRAINING [GENERATION] Starting AI content generation for '{name}'")
                ...
                try:
                    ...
                    logger.info(f"✅ AI_TRAINING [GENERATION] AI content generated - {len(ai_content)} characters")
                    ...
                    logger.info(f"✅ AI_TRAINING [STORAGE] AI content stored - {file_size} bytes")
                    
                except AITrainingGenerationError as ai_error:
                    logger.error(f"❌ AI_TRAINING [ERROR] AI generation failed: {str(ai_error)}")
                    
                    # Create fallback content for graceful degradation
                    fallback_content = f"""# {name}

## Formation générée par IA - Indisponible temporairement
...
Le service de génération automatique de contenu de formation n'est pas disponible actuellement. 

**Solutions alternatives :**
1. Contactez votre formateur pour obtenir le matériel de formation
2. Réessayez plus tard lorsque le service sera rétabli
3. Créez une nouvelle formation avec un fichier PDF/PowerPoint

---

*Formation créée le {datetime.now().strftime('%d/%m/%Y à %H:%M')}*
...
"""
                    ...
                    logger.info(f"📄 AI_TRAINING [FALLBACK] Fallback content stored - {file_size} bytes")
